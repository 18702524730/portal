<template>
  <div>
    <p v-if="msTime.show">
      ...
      <i class="el-icon-time" v-if="iconState"></i>剩余<span v-if="msTime.day>0"><span>{{msTime.day}}</span><i>{{dayTxt}}</i></span><span>{{msTime.hour}}</span><i>{{hourTxt}}</i><span>{{msTime.minutes}}</span><i>{{minutesTxt}}</i>
