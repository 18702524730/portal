...
export default {
  ...
  methods: {
    ...
    jumpLogin(){
      var url = location.href;
      location.href = userUrl + '/iprp/#/access/wechat?return_url=' + encodeURIComponent(url);
    },
  	logout(){
      var self = this;
      ...
      .then((response) => {
        ...
        //self.jumpLogin();
        //登录后回到首页
        location.href = userUrl + '/iprp/#/access/wechat?return_url=' + encodeURIComponent(indexUrl);
      })
      .catch((error) => {
        self.clearCookie();
        if (!error.response) {
    ...
    cartNavTo(){
      location.href = this.userName ? cartUrl+'/shop/pages/buycart.html' : userUrl + '/iprp/#/access/wechat?return_url=' + encodeURIComponent(cartUrl+'/shop/pages/buycart.html');
    },
    // 切换商家服务中心
    serviceClick(type){
      this.topIndex = true;
        ...
        if (data.spExists && data.applyExist) {
          ...
          setTimeout(()=>{
            ...
            if (location.host.indexOf('localhost') === 0) {
              ...
              location.href = decodeURIComponent(this.spUrl + addr);
            }
          },600);
          return
        }
