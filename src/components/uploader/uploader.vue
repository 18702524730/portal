<template>
    ...
    <el-dialog :visible.sync="dialogImgVisible" custom-class="previewDialog previewDialog_trans" :modal-append-to-body="true" :lock-scroll="true" top="50%" :modal="isDialog? false: true" ref="previewDialog" :close-on-click-modal="false">
  ...
  function ElementSupportAttribute(elm, attr) {
    ...
    props:{
      ...
      // action
    	postAction:{
    		type: String,
    		default: rootUrl + '/api/file/uploadImgToPdf'
    	},
      // name
    	name: {
    		type: String,
    ...
    methods: {
      ...
      inputFile(newFile, oldFile) {
        ...
        if (newFile && oldFile) {
          ...
          //上传成功
          if (this.progress == '100.00' && res && (res.url || res.code || res.status)) {
...
<style lang='scss'>
    ...
    .previewDialog{
    	width: 900px!important;min-height: 700px;
    	.el-carousel{
    ...
    .previewDialog_trans{
      ...
      .el-carousel{
        button{
        	width: 36px;height: 36px;border-radius:50%;background-color:rgba(31,45,61,.11);
        	i{vertical-align: top;line-height: 36px;}
        }
      }
    }
	}
