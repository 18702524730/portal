					...
					<transition-group name="fade" tag="div">
						<div class="state" :class="{'mt20': $draftIndex==0}" v-for="(info,$draftIndex) in draftInfo" :key="info.class_id" :id="'info_box_'+$draftIndex">
							<div class="state_list" :class="{'bgc':$draftIndex === draftIndex}">
								...
								<!-- 展开 -->
								<a href="#" class="del" v-if="draftInfo && draftInfo.length > 1" @click.prevent="askDelete($draftIndex)"><i class="el-icon-delete"></i></a>
								<img class="unwrap" src="~assets/img/selfRegister/release.png" alt="" v-if="info.PackUp" @click="openCurOrder($draftIndex,info)">
								<!-- 收缩 -->
								<img src="~assets/img/selfRegister/shrinkage.png" alt="" v-if="!info.PackUp" @click="closeCurOrder(info)">
	...
	export default {
		...
		methods: {
			...
			saveDraft(){
				...
				this.$http.post(spUrl+'/iprp_servicer/api/commitWorkOrderNiceRec',info)
				.then((resp)=>{
    	...
    	deleteBig(){
			...
			queryDetail(){
				...
				console.log(key)
				this.$http.get(spUrl+'/iprp_servicer/api/findWorkOrderNiceRec?key='+key)
				.then((resp)=>{
...
<style lang='scss'>
	.step1_fill_wrap{background:url(~assets/img/nice/niceBgi.png) center top;padding-top:84px;padding-bottom: 65px;font-size:0;min-height: 100%;
			...
			.el-dialog{
				...
				.el-dialog__body{
					.confirm_tip{
						...
						a.no{border: 1px solid #dcdfe6;margin-right: 20px;}
						a.yes{background: #3db1fa;color: #fff;border: 1px solid #3db1fa;}
					}
	...
	.state_preview{
		.el-dialog{
			width:310px;top:10%;position: absolute;left: 50%;transform: translate(-50%, -50%);
			.confirm_tip{
