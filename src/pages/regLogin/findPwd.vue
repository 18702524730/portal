    ...
    <div class="find_passward">
      <div class="find_passward_in commWidth register_box">
        ...
        <div class="find_passward_box">
          <ul class="steps slearfix" v-if="step != 4">
          ...
          <!-- 第1步 填写账户名 -->
          <el-form :model="postData" :rules="rules1" ref="postData" class="demo-ruleForm" v-if="step === 1">
            <div class="reg_content" >                
              <el-form-item label="" prop="account" class="input">
                <el-input placeholder="请输入注册手机号" v-model="postData.account" maxlength="11" size="medium" clearable ></el-input>
              </el-form-item>
              <el-form-item label="" prop="captcha" class="input_code">
                <el-input placeholder="请输入验证码" v-model="postData.captcha" size="medium" maxlength="4" clearable></el-input>
              ...
              <el-button size="medium" type="primary" class="submit_btn" @click.prevent="saveFirst('postData')">下一步</el-button>
            </div>
          </el-form>

          <!-- 第2步 验证身份 -->
          <el-form :model="postData" :rules="rules2" ref="postData" class="demo-ruleForm" v-if="step === 2">
            <div class="reg_content" >
              <el-form-item class="input">
                <el-input placeholder="请输入注册手机号" v-model="postData.account" class="input" disabled size="medium" clearable></el-input>
              </el-form-item>
              <el-form-item class="input input_code" prop="code">
                <el-input placeholder="请输入验证码" v-model="postData.code" size="medium" ></el-input>
                <span class="getCode" :class="{disabled: preventCode}" @click.prevent="getCode">{{codeText}}</span>
              </el-form-item>
              <el-button size="medium" type="primary" class="submit_btn" @click.prevent="saveSecond('postData')">验证</el-button>
            </div>
          </el-form>

          <!-- 第3步 设置新密码 -->
          <el-form :model="pwdObj" :rules="rules3" ref="pwdObj" class="demo-ruleForm" v-if="step === 3">
            <div class="reg_content" >
  ...
  const rootUrl = CONFIG.rootUrl;
    ...
    components: {
      ...
      let validateCaptcha = (rule, value, callback) => {
        if (!/^\w{4}$/.test(value)) {
          callback(new Error('请输入4位数字或字母的验证码'));
        } else {
          callback();
        };
      }
      ...
      let validatePhone = (rule, value, callback) => {
        if (!/^1[0-9]{10}$/.test(value)) {
          ...
          callback();
			...
			return{
        stepsData:['填写注册手机号', '短信验证', '设置新密码', '完成'],
        step: 1,
        codeUrl: rootUrl,
        random: Math.random(),
        ver_tip:'',
        ...
        self.$refs[formName].validate((valid) => {
          ...
          if (valid) {
            ...
            self.$http.post('./api/member/updateFindPassword', {
              ...
              new_password: md5(this.pwdObj.newPassword)
            })
            .then((response) => {
              console.log(response.data)
              self.step = 4;
...
.find_passward{
  ...
  .reg_content {
    ...
    .line {
      ...
      .input_code {width:160px; vertical-align: middle;}
      .captcha-span{ 
        height: 25px;display: inline-block;padding-left:10px;top:6px; vertical-align: middle; margin-left: 10px;
        img {cursor: pointer;}
      }
      .getCode {
        float: right;
